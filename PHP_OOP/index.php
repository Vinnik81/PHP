...
class Battle implements IBattle {
    ...
    private function fight($attaker, $loozer) {
        $hp = rand(1, 100);
        $loozer->downXp($hp);
        echo $attaker->getName().' бьёт '. $loozer->getName().' на '. $hp.'хп, осталось ('.$loozer->hp().' хп)'."</br>";
        if (!$loozer->ISLive()) {
            unset($this->beings[array_search($loozer, $this->beings)]);
            sort($this->beings);
            echo $loozer->getName().' - был побежден!'."</br>";
        }
        $this->selected();
    }
}

$being = new Being('Vital', 300);
$being1 = new Being('Vulf', 300);
$being2 = new Being('Pasher', 300);
$being3 = new Being('Sash', 300);
